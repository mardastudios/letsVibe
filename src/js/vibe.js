...
async function addStreamtoPeerConnection(pc) {
	...
	$(".audioComponent").append(localAudio);
...
function answerCall(pub) {
	...
	initConnection(false, pub);
}
function callUser(pub) {
	var end_vibe = $(
		'<button id="end-vibe" type="btn"><i class="material-icons" aria-hidden="true">call_end</i> End Vibe</button>'
	);
	initConnection(true, pub);
...
async function initConnection(createOffer, pub) {
	...
	friends[pub].pc.ontrack = (event) => {
		...
		if (remoteAudio.src !== event.streams) {
			...
			createResonanceScence(event.streams);
			remoteAudio.src = event.streams;
			remoteAudio.onloadedmetadata = function () {
				console.log("metadata loaded");
			};
			$(".audioComponent").append(remoteAudio);
